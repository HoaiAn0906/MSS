...
export function mapProductFormDataToProductPayload(
  ...
  return {
    ...
    metaKeyword: data.metaKeyword ?? undefined,
    ...
    variations: data.productVariations
      ...
      : [],
    productOptionValues: createProductOptionValues(
      data.productVariations || []
    ),
    productOptionValueDisplays: createProductOptionValueDisplay(
      data.productOptionValuePost || []
    ),
...
const createProductOptionValueDisplay = (
...
const createProductOptionValues = (productVariations: ProductVariation[]) => {
  const productOptionValues: ProductOptionsValuePost[] = [];
  productVariations.forEach((variation) => {
    const option = variation.optionValuesByOptionId;
    Object.entries(option).forEach((entry) => {
      const id = +entry[0];
      const value = entry[1];
      const optionValue = productOptionValues.find((option) => {
