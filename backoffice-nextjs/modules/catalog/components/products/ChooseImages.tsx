...
export default function ChooseImages({
  ...
  const onUploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (name === "productImageMedias") {
        ...
        if (variation.optionImages)
          variation.optionImages = [...variation.optionImages, ...images];
        else {
  ...
  const onDeleteImage = (imageId?: number) => {
    if (name === "productImageMedias") {
      ...
      variation !== undefined &&
      variation.optionImages
    ) {
      variation.optionImages = variation.optionImages.filter(
