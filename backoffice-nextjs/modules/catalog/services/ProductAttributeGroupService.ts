...
export async function getProductAttributeGroups(): Promise<
  ProductAttributeGroup[]
> {
  return (await apiClientService.get(baseUrl)).json();
}

export async function getPageableProductAttributeGroups(
  pageNo: number,
  pageSize: number
) {
...
export async function createProductAttributeGroup(
  productAttributeGroup: ProductAttributeGroup
) {
  return await apiClientService.post(
    baseUrl,
    JSON.stringify(productAttributeGroup)
  );
}
